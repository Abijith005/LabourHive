...
export class RatingComponent implements OnDestroy {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) private  _data:{labour_id:string,hire_id:string},
...
this._profileService.postReview(this.review,this.rating,this._data.labour_id,this._data.hire_id).pipe(takeUntil(this._unsubscribe)).subscribe(res=>{
  console.log(res);
  if (res.success) {
