...
export class AdminJobManagementService {
  ...
  getJobDetails(
    ...
    return this._http.post<i_allJobs[]>(`/admin/getAllJobDetails`, {
      filter,
      search,
      page,
      startDate,
  ...
  changeJobStatus(job_id: string, status: string) {
    return this._http.patch<i_authRes>(`/admin/changeJobStatus`, {
      job_id,
      status,
    });
  }
  ...
  updatePayment(
    ...
    return this._http.patch<i_authRes>(`/admin/updatePayment`, {
      hire_id,
      payment,
      user_id,
      amount,
